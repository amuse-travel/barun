...
이 약관은 (주)어뮤즈 (이하 "회사") 에서 제공하는 "바른연애생활"에서 제공하는 제반 서비스(이하 "서비스")에 접속과 사용자에 의해서 업로드 및 다운로드 되어 표시되는 모든 정보, 텍스트, 이미지 및 기타 자료를 이용하는 이용자(이하 "회원")와 서비스 이용에 관한 권리 및 의무와 책임사항, 기타 필요한 사항을 규정하는 것을 목적으로 합니다. 

제 2조 용어의 정의

- 서비스: 개인용 컴퓨터 (PC), TV, 휴대형 단말기, 전기통신설비 등 포함 각종 유무선 장치와 같이 구현되는 단말기와 상관없이 회원이 이용할 수 있는 "바른연애생활" 관련 제반 서비스를 의미합니다.- 회원: 회사와 서비스 이용계약을 체결하고 회사가 제공하는 서비스를 이용하는 모든 사용자를 의미합니다.- 아이디: 회원이 가입할 때 입력한 회원 본인의 이메일 주소를 의미합니다.- 비밀번호: 회원의 개인 정보 및 확인을 위해서 회원이 정한 문자 또는 숫자의 조합을 의미합니다.- 도메인: 회원의 서비스 이용을 위하여 회사가 부여한 고유한 인터넷 주소를 의미합니다. 회사는 제공하는 제반 서비스를 위해서 서비스에 따라 별도의 도메인 주소를 추가적으로 제공합니다.- 유료서비스: 회사가 유료로 제공하는 각종 온라인디지털콘텐츠 및 제반 서비스를 의미합니다. - 리본: 서비스를 이용 또는 구매하기 위해 사용되는 가상의 화폐 단위로서 회원이 대금을 지급하고 구입하는 것을 의미합니다. 

제 3조 이용약관의 효력 및 변경

1. 이 약관은 서비스를 이용하고자 하는 모든 회원에 대하여 그 효력이 발생합니다.
2. 이 약관의 내용은 서비스를 통해 게시하여 회원에게 공시하고, 이에 동의한 회원이 서비스에 가입함으로써 효력이 발생합니다.
3. 사업자가 약관을 변경할 경우에는 적용일자 및 변경사유를 명시하여 현행약관과 함께 서비스초기화면에 그 적용일 최소한 7일 이전부터 적용일 후 상당한 기간동안 공지한다. 다만 이용자에게 불리하거나 중대한 사항을 변경할 경우에는 최소한 30일 이전부터 공지하며, 기존이용자에게는 변경될 약관, 적용일자 및 변경사유를 이메일, 문자메시지 등으로 고지한다.
4. 사업자가 약관을 변경할 경우에는 제3항의 공지 및(또는) 고지와 함께 또는 그 후에 변경약관의 적용에 대한 이용자의 동의 여부를 확인한다. 이 경우 동의나 거절의 방법을 위 공지 및(또는) 고지 시에 함께 알려야 한다.
...
이 약관에 명시되지 않은 사항에 대해서는 콘텐츠산업진흥법, 전자상거래 등에서의 소비자보호에 관한 법률, 정보통신망 이용촉진 및 정보보호 등에 관한 법률 등 관계법령 및 회사가 정한 서비스의 세부 이용 지침 등의 규정에 의합니다.개인정보 및 위치정보에 관해서는 개인정보 처리방침 및 위치기반 이용약관이 적용됩니다.

02 서비스 이용 계약

제 5조 이용계약의 성립

1. 이용계약은 회원이 "바른연애생활" 서비스 및 제반 서비스에서 제공하는 회원가입 페이지에서 서비스 이용약관에 동의한 후 이용신청을 하고 신청한 내용에 대해서 회사가 승낙함으로써 체결됩니다.
...
3. 회사는 회원이 서비스 내에 게시하거나 전달하는 모든 내용물(회원간 전달 포함)이 다음 각 호에 해당한다고 판단될 경우 사전 통보 없이 게시 중단, 이동, 삭제 등의 조치를 취할 수 있고, 필요하다고 판단되는 경우 해당 회원의 서비스 이용 자격을 제한하거나 정지, 상실시킬 수 있습니다.
...
제 14조 리본, 포인트 및 아이템의 사용, 소멸 및 관리

1. 회사는 회원의 일정한 조건에 해당하는 활동에 대하여 리본를 부여하고 관리합니다.
2. 회원이 부정한 방법으로 리본 및 등급을 획득한 사실이 확인될 경우 회사는 리본 회수, 등급 조정 및 이용 제한 조치를 취할 수 있습니다.
3. 회사는 회원이 본 약관을 위반한 경우 기타 운영상의 합리적인 사유가 있을 경우 회원의 리본를 삭감하거나 등급을 조정할 수 있습니다.
4. 리본 및 아이템은 회사가 제공하는 서비스를 사용할 때 지불 수단으로 이용될 수 있습니다.
5. 회원이 구매한 리본 및 아이템은 해당 회원의 최종 접속일로부터 2년 경과 시 소멸됩니다. 단, 구매한 아이템의 사용 기간이 정해져 있는 경우 해당 기간 내에서만 사용이 불가능합니다.
6. 회사가 회원에게 무상으로 지급한 리본 및 아이템은 해당 회원의 최종 접속일로부터 2년 경과 시 소멸됩니다. 단, 아이템의 사용 기간이 정해져 있는 경우에 해당 기간 내에서만 사용이 가능합니다.
7. 리본의 사용 순서는 회사가 정하는 정책에 따릅니다.
8. 회원의 리본 및 아이템 구매내역은 회사의 시스템 DB에 저장되어 구매 및 구매 이후에 생기는 문제를 관리 할 수 있도록 합니다.
9. 회원의 리본 및 아이템 구매가 서비스의 결함 및 장애 등으로 유실 될 경우 회사는 시스템 DB 를 확인 후 보상 할 수 있도록 합니다.
...
1. 회사는 계속적이고 안정적인 서비스의 제공을 위하여 최선을 다하여 노력합니다.2. 회사는 회원의 개인정보 보호를 위해 보안시스템을 구축하며 개인정보 처리방침을 공시하고 준수합니다.3. 회사는 서비스와 관련한 회원의 정당한 불만 혹은 의견을 접수하기 위하여 전담 메뉴를 운영하며 회원으로부터 제기되는 의견이나 불만이 정당하다고 인정할 경우, 이를 처리합니다.4. 회사는 회원의 개인정보를 본인의 동의없이 제3자에게 제공하지 않으며 “개인정보취급방침”을 공지하고 준수하며 회원정보를 취급함에 있어 안정성 확보 및 기술적 대책을 수립 운영합니다.5. 회사는 유료서비스 제공시 유료서비스의 내용, 이용방법, 이용료, 결제방법 등을 명확히 공지해야 합니다.6. 회사는 정보통신망 이용촉진 및 정보보호에 관한 법률, 통신비밀보호법, 전기통신사업법 등 서비스의 운영, 유지와 관련 있는 법규를 준수합니다.
...
- 기타 "바른연애생활" 모바일 어플리케이션 또는 온라인 웹 페이지를 통하여 추가로 공지되는 사항에 위배되는 경우
